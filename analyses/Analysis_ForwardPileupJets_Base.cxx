/**************************************************************************
 **
 **   File:         Analysis_ForwardPileupJets_Base.h
 **
...
#define Analysis_ForwardPileupJets_Base_cxx
...
// start with some jet collection. specify a list of constituents or other objects.
// will make a new jet collection, where jets are composed of the consituents specified.
MomKey Analysis_ForwardPileupJets_Base::AddRecomputedJets(const MomKey JetKey, const MomKey CollectionKey){
  const static MomKey mJet("Jet");
  const static MomKey mjets("jets");
  const static MomKey mconstituents("constituents");


  AddVec(mjets+JetKey+CollectionKey);

  for(int iJet = 0; iJet < jets(JetKey); iJet++){
    Particle* theJet = &jet(iJet, JetKey);
    Particle* newJet = new Particle();

    theJet->AddVec(CollectionKey+mJet);
    Add(mjets+JetKey+CollectionKey, newJet);
    theJet->Add(CollectionKey+mJet, newJet);

    newJet->AddVec(mconstituents);

    for(int iC = 0; iC < theJet->Objs(CollectionKey); iC++){
      Particle* theCons = (Particle*) theJet->Obj(CollectionKey, iC);
      newJet->p += theCons->p;
      newJet->Add(mconstituents, theCons);
    }
  }

  return JetKey+CollectionKey;
}


bool Analysis_ForwardPileupJets_Base::AddGrid(const MomKey ObjectKey){
  const MomKey GridKey = ObjectKey + MomKey("Grid");
  const static MomKey mEta("Eta");
  const static MomKey mPhi("Phi");
  const static MomKey mE("E");


  AddVec(GridKey);
  //et setup the grid, initialize
  for(int iPhi = -32; iPhi < 32; iPhi++){
    for(int iEta = -50; iEta < 50; iEta++){
      float phiVal = 0.10 * ((float) iPhi) + 0.05;
      float etaVal = 0.10 * ((float) iEta) + 0.05;

      Particle* gridPoint = new Particle();
      gridPoint->Set(mEta, etaVal);
      gridPoint->Set(mPhi, phiVal);
      gridPoint->Set(mE  , 0     );

      Add(GridKey, gridPoint);
    }
  }
  // loop over particles
  for(int iC = 0; iC < Objs(ObjectKey); iC++){
    Particle* theParticle = ((Particle*) Obj(ObjectKey, iC));

    float theEta = theParticle->p.Eta();
    float thePhi = theParticle->p.Phi();
    float theE   = theParticle->p.E();

    if(theEta >= 4.95 || theEta < -4.95)
      continue;

    // find the matching grid point
    for(int iPhi = -32; iPhi < 32; iPhi++){
      for(int iEta = -50; iEta < 50; iEta++){
        float phiVal = 0.10 * ((float) iPhi) + 0.05;
        float etaVal = 0.10 * ((float) iEta) + 0.05;

        if((theEta >= (etaVal - 0.05) && theEta < (etaVal + 0.05)) &&
           (thePhi >= (phiVal - 0.05) && thePhi < (phiVal + 0.05))){
          Particle* gridPoint = 0;
          for(int iGrid = 0; iGrid < Objs(GridKey); iGrid++){
            Particle* currentGrid = (Particle*) Obj(GridKey, iGrid);
            if(currentGrid->Float(mEta) == etaVal &&
               currentGrid->Float(mPhi) == phiVal){
              gridPoint = currentGrid;
            }
          }
          // if a grid point is found, add energy of the particle to that point
          if(gridPoint){
            gridPoint->Set(mE, gridPoint->Float(mE) + theE);
          }
        }
      }
    } 
  ...
  // loop over points, create 4 vectors
  for(int iG = 0; iG < Objs(GridKey); iG++){
    Particle* gridPoint = ((Particle*) Obj(GridKey, iG));
    gridPoint->p.SetPtEtaPhiM(gridPoint->Float(mE) / TMath::CosH(gridPoint->Float(mEta)),
                              gridPoint->Float(mEta), 
                              gridPoint->Float(mPhi),
                              0.);
  }
...
bool Analysis_ForwardPileupJets_Base::ManuallyAddConstituents(const MomKey JetKey){
...
bool Analysis_ForwardPileupJets_Base::SumMatchedCollectionJets(const MomKey JetKey, const MomKey collection){
  for(int iJet = 0; iJet < jets(JetKey); iJet++){
...
bool Analysis_ForwardPileupJets_Base::SumMatchedCollection(Particle* theJet, const MomKey collection){
...
bool Analysis_ForwardPileupJets_Base::ReduceTruthBHadronCollection(const MomKey TruthKey){
...
bool Analysis_ForwardPileupJets_Base::LabelBJetsDR(const MomKey JetKey){
...
///=========================================
void Analysis_ForwardPileupJets_Base::PrintParticle(Particle* part){
  if(!isMC()) 
    return;
...
bool Analysis_ForwardPileupJets_Base::GetBHadronDecayProducts(Particle* part, bool printFinalProducts, bool printTree)
{
  if(!isMC()) 
    return false;
...
bool Analysis_ForwardPileupJets_Base::MakeTruthBHadronCollection()
{

  if(Debug()) cout << "Entering Analysis_ForwardPileupJets_Base::MakeTruthBHadronCollection" << endl;
  
  if(!isMC()) 
    return false;
  
  if(truths()<=0){
    std::cout<<"Analysis_ForwardPileupJets_Base::MakeTruthBHadronCollection - No Truth particles found???"<<std::endl;
    return false;
  }    
...
///=========================================
bool Analysis_ForwardPileupJets_Base::IsBHadron(int pdgId)
{
...
///=========================================
bool Analysis_ForwardPileupJets_Base::IsWeaklyDecayingBHadron(int pdgId)
{
...
bool Analysis_ForwardPileupJets_Base::IsBHadronFromTop(int iTruth)
{
  if(Debug()) cout << "Entering Analysis_ForwardPileupJets_Base::IsBHadronFromTop" << endl;
      ...
      for(unsigned int iPar=0; iPar<curr_particle->ObjVec("parents").size(); iPar++)
  {
    ...
    //  cout << "Warning in Analysis_ForwardPileupJets_Base::IsBHadronFromTop: The BHadron goes back to itself through parent chain!" << endl;
    //  goodParent = false;
    //break;
    //}
  ...
  if(Debug()) cout << "Leaving Analysis_ForwardPileupJets_Base::IsBHadronFromTop" << endl;
...
MomKey Analysis_ForwardPileupJets_Base::GetBTagKey(const int SystType){
  MomKey mSys;
...
MomKey Analysis_ForwardPileupJets_Base::GetMuonSFKey(const int SystType){
  MomKey mSys; 
...
MomKey Analysis_ForwardPileupJets_Base::GetMuonTrigSFKey(const int SystType){
  MomKey mSys; 
...
void Analysis_ForwardPileupJets_Base::DoBtaggingSF(bool sys, const MomKey JetType)
{
...
void Analysis_ForwardPileupJets_Base::DoMuonSF(){
...
void Analysis_ForwardPileupJets_Base::MakeDRMatch(const MomKey JetKey1, const MomKey JetKey2){
...
void Analysis_ForwardPileupJets_Base::AddGhostMatch(const MomKey JetKey, const MomKey MatchObject){
...
bool Analysis_ForwardPileupJets_Base::AddDRMatch(const MomKey JetKey, const MomKey MatchObject){
  float baseR = 0.4;
...
void Analysis_ForwardPileupJets_Base::AddGhostMatch(const MomKey JetKey, const MomKey MatchObject, const MomKey ConstType, const fastjet::JetAlgorithm AlgoIn, const float Radius){
  ...
  for(int iJet = 0 ; iJet < jets(JetKey) ; iJet++){
    ...
    TLorentzVector referenceLV;
    referenceLV.SetPtEtaPhiM(jet(iJet,JetKey).Float("constscale_pt") , jet(iJet,JetKey).Float("constscale_eta"), jet(iJet,JetKey).Float("constscale_phi"), jet(iJet,JetKey).Float("constscale_m"));
    fastjet::PseudoJet referencejet;
    referencejet.reset_PtYPhiM(referenceLV.Pt(), referenceLV.Rapidity(), referenceLV.Phi(), referenceLV.M());
    for(int ifj=0; ifj<inclusiveJets.size(); ++ifj){
        fastjet::PseudoJet jetFJ = inclusiveJets[ifj];
        if(!(jetFJ.delta_R(referencejet) < 0.2 && fabs(jetFJ.pt() - referencejet.pt())/referencejet.pt() < 0.05)) continue;
        ...
        jet(iJet, JetKey).SortPt(MatchObject+Ghost); // sort ghost associated particles by pt
...
void Analysis_ForwardPileupJets_Base::AddAsGhosts(const MomKey GhostObject, vector<fastjet::PseudoJet>* TheThings, bool IsTrack){
...
void Analysis_ForwardPileupJets_Base::AddTruthStableCharged(const MomKey JetKey){
...
void Analysis_ForwardPileupJets_Base::CalculateResponseOffset(const MomKey JetKey, const MomKey MatchKey){
...
void Analysis_ForwardPileupJets_Base::MakeDRResponseOffset(const MomKey JetKey, const MomKey TruthJetKey){
  MakeDRMatch(JetKey, TruthJetKey);
  const static MomKey DRMatch("DRMatch");
  CalculateResponseOffset(JetKey, DRMatch+TruthJetKey);
}

void Analysis_ForwardPileupJets_Base::WorkerBegin(){

}

void Analysis_ForwardPileupJets_Base::RemoveJetsPtCut(const MomKey JetType, float PtCut){
...
MomKey Analysis_ForwardPileupJets_Base::AddSubjets(const MomKey JetType, const fastjet::JetAlgorithm algo, const double fcut, const double ptcut){
...
MomKey Analysis_ForwardPileupJets_Base::SetEventSubjets(const MomKey JetType, const MomKey SubjetType, int NJets){
  int nsub = 0;
...
MomKey Analysis_ForwardPileupJets_Base::SetEventSubjettiness(const MomKey JetType, const MomKey nsubType, const int NJets){
  float nsubprod = 1.;
...
MomKey Analysis_ForwardPileupJets_Base::SetEventMass(const MomKey JetType, int NJets){
  float massTotal = 0.;
...
MomKey Analysis_ForwardPileupJets_Base::SetHt(const MomKey JetType, int NJets){
...
void Analysis_ForwardPileupJets_Base::SetFatJetOverlap(const MomKey FatKey, const MomKey SmallKey, float dR){
...
///=========================================
vector<fastjet::PseudoJet> Analysis_ForwardPileupJets_Base::ObjsToPJ(const MomKey type, MomentObj* jet){
  jet = (jet!=0)?jet:this;
...
MomKey Analysis_ForwardPileupJets_Base::MakePrunedJetsName(const MomKey JetType, const fastjet::JetAlgorithm algo, const double zcut, const double dcut){
...
MomKey Analysis_ForwardPileupJets_Base::MakePrunedJets(const MomKey JetType, const fastjet::JetAlgorithm algo, const double zcut, const double dcut){
...
MomKey Analysis_ForwardPileupJets_Base::MakeJets(const fastjet::JetAlgorithm algo, const double jetR, const MomKey constType, const MomKey extra){
...
///=========================================
void Analysis_ForwardPileupJets_Base::AddNsub(const MomKey constType, const MomKey jetCollection, const bool min, const float beta){
...
void Analysis_ForwardPileupJets_Base::CalcNsub(Particle* myjet, const MomKey constType, const bool min, const float beta, const float jetRad){
	//if(Debug()) myjet->Show();
...
////////////////////////////////////////////////////////
MomKey Analysis_ForwardPileupJets_Base::AddStableParticles(){
...
////////////////////////////////////////////////////////
MomKey Analysis_ForwardPileupJets_Base::AddGoodTracks(){
...
TH1* Analysis_ForwardPileupJets_Base::Fill(AnaKey name, double XVal, double weight, int NumBinsX, double XMin, double XMax){
	...
	TString msg("Analysis_ForwardPileupJets_Base: ERROR No valid object ");
	msg.Append(name);
	msg.Append(" found");
	Abort(msg);		
...
TH1* Analysis_ForwardPileupJets_Base::Fill(AnaKey name, double XVal, double weight, int NumBinsX, const float* Xbins){
...
TH1* Analysis_ForwardPileupJets_Base::Fill(AnaKey name, std::string label, double weight, std::vector<std::string> x_labels){
...
TH2* Analysis_ForwardPileupJets_Base::Fill(AnaKey name, double XVal, double YVal, double weight, 
...
TH2* Analysis_ForwardPileupJets_Base::Fill(AnaKey name, std::string xlabel, std::string ylabel, double weight,
...
TH3* Analysis_ForwardPileupJets_Base::Fill(AnaKey name, double XVal, double YVal, double ZVal, double weight, 
