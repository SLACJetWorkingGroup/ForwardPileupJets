...
#ifndef Analysis_ForwardPileupJets_Base_h
#define Analysis_ForwardPileupJets_Base_h
...
class Analysis_ForwardPileupJets_Base : public AnalysisBase {

public :

  Analysis_ForwardPileupJets_Base(TTree* /*tree*/ = 0) { 
    fDetail = false; 
  }
  virtual ~Analysis_ForwardPileupJets_Base() { }    
  
  ClassDef(Analysis_ForwardPileupJets_Base, 0);


  Bool_t  fDetail;
  
  virtual bool    ProcessEvent(){/* To be overridden by derived analysis class */ 
    std::cout<<"%%%%% Analysis_ForwardPileupJets_Base::ProcessEvent() %%%%%%%%%%%%%"<<std::endl;
    return kTRUE;};
  ...
  // make a 0.1 x 0.1 grid
  bool                          AddGrid(const MomKey ObjectKey);


  // make a new jet collection out of things associated to a set of jets
  MomKey                        AddRecomputedJets(const MomKey JetKey, const MomKey CollectionKey);
