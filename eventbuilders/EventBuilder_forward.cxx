...
#define EventBuilder_forward_cxx
/// Type used for internal caching

#include "EventBuilder_forward.h"
#include "AnaConfig.h"
...
EventBuilder_forward::EventBuilder_forward(TTree * /*tree*/)
{
...
EventBuilder_forward::~EventBuilder_forward(){
  // nothing to delete
  //delete m_trigdefs;
...
Bool_t EventBuilder_forward::SetupPRW(){
  if(!fEvt->Cfg()->Exists(mPRWVEC)) {
    ...
    for(Int_t i = 0; i<arr->GetEntries(); ++i) {
      ...
      if (fEvt->Debug()) {
        cout << "EventBuilder_forward::CopyEvent(): DEBUG Adding prw type: " << prwType << endl; 
      }
...
void EventBuilder_forward::Initialize(){
  myGRL = (Root::TGoodRunsList*)fInput->FindObject("myGRL");  
  if(!myGRL) {
    cout << "EventBuilder_forward: ERROR cannot retrieve GRL object from input list!" << endl;
    exit(1);
  }
  ...
  //MCPSmear->UseScale(1);
  //MCPSmear->UseImprovedCombine();
  //MCPSmear->RestrictCurvatureCorrections(2.5);
  //MCPSmear->FillScales("KC");
  //MCPSmear->UseGeV();
...
Bool_t EventBuilder_forward::CopyEvent(AnalysisBase* evt)
{
...
////////////////////////////////////////////////////////////
  ...
  evt->Cfg()->Get("DOTWOTRACKVTX",        doTwoTrackVertex);
  ...
  evt->Cfg()->Get("DOGBBCOMMON",          doGbbCommon);
...
////////////////////////////////////////////////////////////
  ...
  if(fEvt->isMC()) {
    //if(!doGbbCommon){ // TODO: add AntiKt6TopoEM. Add all branches related here
    if(!KillPileupFluctuations())
      return kFALSE;
    //}
    if(!DoHFOverlap()) 
      return kFALSE;
...
////////////////////////////////////////////////////////////
  ...
  if(doGbbCommon){
    fail = fail && BuildSubjetLink();
  }

  if(evt->Debug()){
    cout << "Showing at the end of BuildSubjetLink" << endl;
    evt->Show();
  }
...
/// CopyMET
///=========================================
Bool_t EventBuilder_forward::CopyMET(){
...
/// CopyTruthFull
///=========================================
Bool_t EventBuilder_forward::CopyTruthFull()
{
  ...
  static const BranchKey bmc_pt("mc_pt"), bmc_eta("mc_eta"), bmc_phi("mc_phi"), bmc_m("mc_m"),
    ...
    bmc_vx_x("mc_vx_x"), bmc_vx_y("mc_vx_y"), bmc_vx_z("mc_vx_z");
  ...
  static const MomKey index("index"), type("type"), pdgId("pdgId"), charge("charge"), barcode("barcode"), mpt("pt"), status("status"), vx_barcode("vx_barcode"),
    vx_x("vx_x"), vx_y("vx_y"), vx_z("vx_z");


  barcode_link = 0;

  if (fEvt->Debug()) cout << "EventBuilder_forward: DEBUG CopyTruthFull()  and doTruth = " << doTruth << endl;  
  ...
  if(fEvt->isMC() && doTruth) {
    ...
    for (int i = 0; i< Get<Int_t>("mc_n"); i++) {
      ...
      if(doGbbCommon){ // vtx of mc truth particle
        // origin vertex
        Point* mc_vx_origin = new Point();
        mc_vx_origin->x.SetXYZ(Get<vector<float> >(bmc_vx_x).at(i), Get<vector<float> >(bmc_vx_y).at(i), Get<vector<float> >(bmc_vx_z).at(i));

        mc->AddVec("mc_vx_origin");
        mc->Add("mc_vx_origin", mc_vx_origin);
      }
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forward::CopyTruthFull(): DEBUG Scheduled Truth Particles" << endl;
  return kTRUE;
}
...
/// AddTruthParentChild
///=========================================
Bool_t EventBuilder_forward::AddTruthParentChild()
{
  TString prefix;
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG Starting" << endl;

  if(fEvt->isMC() && doTruthParentChild)
  {
    fEvt->AddVec("finaltruths", true);

    for(int iTr = 0; iTr < fEvt->truths(); iTr++)
    {
      if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG On particle " << iTr << " of " << fEvt->truths() << endl;

      fEvt->truth(iTr).AddVec(children, true);
      fEvt->truth(iTr).AddVec(parents, true);
      if(doGbbCommon){
        fEvt->truth(iTr).AddVec("mc_vx_decay", true);
      }

      if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG Has children " << Get<vector<vector<int> > >(bmc_children).at(iTr).size()  << endl;

      int nChildren = Get<vector<vector<int> > >(bmc_children).at(iTr).size();

      // add final truths here
      if(nChildren == 0){
        fEvt->Add("finaltruths", &(fEvt->truth(iTr)));
      }

      for(int iChild = 0; iChild < nChildren; iChild++)
      {
        int index = Get<vector<vector<int> > >(bmc_children).at(iTr).at(iChild);
        if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG Adding child " << index  << endl;
           
        if(!doCOMMON){     
          if((index > -1) && (index < fEvt->truths()) && (index != iTr))
            fEvt->truth(iTr).Add(children,&(fEvt->truth(index)));

            // add decay vertex here for gbbCommon ntuple
            if(doGbbCommon){
              Point* mc_vtx_decay = fEvt->truth(index).Obj<Point>("mc_vx_origin");
              fEvt->truth(iTr).Add("mc_vx_decay", mc_vtx_decay);

              //cout << "parent origin vtx:" << endl;
              //Point* mc_vtx_origin = fEvt->truth(iTr).Obj<Point>("mc_vx_origin");
              //cout << "x: " << mc_vtx_origin->x.X() << endl;
              //cout << "y: " << mc_vtx_origin->x.Y() << endl;
              //cout << "z: " << mc_vtx_origin->x.Z() << endl;
              //cout << "child origin vtx (parent decay vtx):" << endl;
              //cout << "x: " << mc_vtx_decay->x.X() << endl;
              //cout << "y: " << mc_vtx_decay->x.Y() << endl;
              //cout << "z: " << mc_vtx_decay->x.Z() << endl;
            }
      ...
      if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG Has parents " << Get<vector<vector<int> > >(bmc_parents).at(iTr).size()  << endl;
      ...
      for(int iParent = 0; iParent < nParents; iParent++)
      {
        ...
        if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG Adding parent " << index << endl;
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forward::AddTruthParentChild(): DEBUG Ending" << endl;
  return kTRUE;
...
Bool_t EventBuilder_forward::CopyClusters(){
  if(doLCCluster){
    ...
    static const MomKey mcellmaxfrac("cellmaxfrac");
    static const MomKey mlongitudinal("longitudinal");
    static const MomKey mlateral("lateral");
    static const MomKey msecondlambda("secondlambda");
    static const MomKey msecondR("secondR");
    static const MomKey misolation("isolation");
    static const MomKey msignificance("significance");
    static const MomKey meng_pos("eng_pos");
    ...
    static const BranchKey bcl_centerlambda(namefix+"_centerlambda");
    static const BranchKey bcl_firstEdens(namefix+"_firstEdens");
    static const BranchKey bcl_cellmaxfrac(namefix+"_cellmaxfrac");
    static const BranchKey bcl_longitudinal(namefix+"_longitudinal");
    static const BranchKey bcl_lateral(namefix+"_lateral");
    static const BranchKey bcl_secondlambda(namefix+"_secondlambda");
    static const BranchKey bcl_secondR(namefix+"_secondR");
    //static const BranchKey bcl_isolation(namefix+"_isolation");
    //static const BranchKey bcl_significance(namefix+"_significance");
    //static const BranchKey bcl_eng_pos(namefix+"_eng_pos");
...
Bool_t EventBuilder_forward::CopyTracks(){
  ...
  if(doTrack){
    ...
    for(int iTr = 0; iTr < Get<Int_t>(btrk_n); iTr++){
      ...
      if(!doTrkJetBTag){
        ...
        if(!doSMWZ && !doCOMMON && !doGbbCommon) // Reminder: Add complete variable sets on track in next Gbb Common version
        ...
        if((doCOMMON || doGbbCommon )&& fEvt->isMC() ){
          ...
          if(tr->Int("mc_index") >= 0){
            tr->AddVec("AssocTruth", true);
            tr->Add("AssocTruth", &(fEvt->truth(tr->Int("mc_index"))));

            fEvt->truth(tr->Int("mc_index")).AddVec("AssocTrack", true);
            fEvt->truth(tr->Int("mc_index")).Add("AssocTrack", tr);
          }
        }
        tr->Set("z0SinTheta",      tr->Float("z0_wrtPV")*TMath::Sin(tr->Float("theta")));
      }
      else{
      ...
      // Track Selection (not a filter)
      tr->Set("isPV", false);
      if(tr->Exists("AssocVtx")){
        if(fEvt->Index("vtxs", tr->Obj<Point>("AssocVtx"))==0){ // HS-PV
          if(fabs(tr->Float("z0SinTheta")) < 1.)  tr->Set("isPV", true);
        }
        else{ // other-PV
          // do nothing -- throw away
        }
      }
      else{ // nonPV-associated
        if(fabs(tr->Float("z0SinTheta")) < 3.) tr->Set("isPV", true);
      }
...
Bool_t EventBuilder_forward::CopyVertices()
{
  ...
  if(doVertex){
    ...
    for(unsigned pvi=0;pvi<Get<vector<int> >(bvxp_nTracks).size();++pvi) {
      if(Get<vector<int> >(bvxp_nTracks).at(pvi) >= 2){
        ...
        for(unsigned int iTrk=0; iTrk<trk_index.size(); ++iTrk){
            vtx->Add("vtxTracks", &(fEvt->track(trk_index[iTrk])), true);
            // trk associate back to vtx
            fEvt->track(trk_index[iTrk]).AddVec("AssocVtx", true);
            fEvt->track(trk_index[iTrk]).Add("AssocVtx", vtx, true);
        }
...
Bool_t EventBuilder_forward::CopyTwoTrackVertices(){
  if(fEvt->Debug()) cout << "Entering EventBuilder_forward::CopyTwoTrackVertices" << endl;

  if(doTwoTrackVertex && doGbbCommon){ // only gbb COMMON_NTUP support two-track vertex this moment
    fEvt->AddVec("twotrackvtxs");

    int ntwotrackvtxs = (int)Get<Int_t>("jftwotrkvertex_n");

    for(int i = 0; i < ntwotrackvtxs; i++){
      Point* twotrackvtx = new Point();

      twotrackvtx->Set("isNeutral", Get<vector<float> >("jftwotrkvertex_isNeutral").at(i));
      twotrackvtx->Set("chi2", Get<vector<float> >("jftwotrkvertex_chi2").at(i));
      twotrackvtx->Set("ndof", Get<vector<float> >("jftwotrkvertex_ndof").at(i));
      twotrackvtx->Set("mass", Get<vector<float> >("jftwotrkvertex_mass").at(i));

      twotrackvtx->x.SetXYZ(Get<vector<float> >("jftwotrkvertex_x").at(i), Get<vector<float> >("jftwotrkvertex_y").at(i), Get<vector<float> >("jftwotrkvertex_z").at(i));
      twotrackvtx->Set("errx", Get<vector<float> >("jftwotrkvertex_errx").at(i));
      twotrackvtx->Set("erry", Get<vector<float> >("jftwotrkvertex_erry").at(i));
      twotrackvtx->Set("errz", Get<vector<float> >("jftwotrkvertex_errz").at(i));

      fEvt->Add("twotrackvtxs", twotrackvtx);
    }

  }

  if(fEvt->Debug()) cout << "Leaving EventBuilder_forward::CopyTwoTrackVertices" << endl;

  return kTRUE;
}

Bool_t EventBuilder_forward::CopyPhotons(){
  if(doPhotons){
    fEvt->AddVec("photons");
...
void EventBuilder_forward::WriteAuxTrees(TDirectory* outfile){
...
/// AddBtags
///=========================================
Bool_t EventBuilder_forward::AddBtags() 
{

  if (fEvt->Debug()) cout << "EventBuilder_forward: DEBUG In AddBtags() " << endl;   
  bool fail = true;
  /// Copy all jet types
  for(Int_t i = 0; i<fEvt->Cfg()->Objs(mBTAGVEC); ++i) {
    ...
    if(fEvt->Debug()) cout << "EventBuilder_forward::AddBtags(): DEBUG Copying btag type " << type << " which is the " << i << "th jet collection" << endl;
    if(fEvt->Debug()) cout << "EventBuilder_forward::AddBtagss(): DEBUG type in vec is " << ((TObjString*) fEvt->Cfg()->Obj(mBTAGVEC,i))->GetString() << endl;
    fail = fail && AddBtag(type,"");
    //delete type;
  }
  
  if (fEvt->Debug()) cout << "EventBuilder_forward::AddBtags(): DEBUG Scheduled All Jet B-tags" << endl;
  return fail;
}
...
/// AddBtag
///=========================================
Bool_t EventBuilder_forward::AddBtag(TString jetType, TString jetClass) 
{

  if (fEvt->Debug()) cout << "EventBuilder_forward: DEBUG In AddBtag() for jetType = " << jetType << "; jetClass = " << jetClass << endl;  
  
  // Handle the case where this is called without an argument
  if (jetType == "") { 
    Abort("EventBuilder_forward::AddBtag(): ERROR No jet type selected. Aborting!");
  }
  
  // Now try to figure out what type of jet this is
  if (jetClass == "") {
    if (fEvt->Debug()) {
      cout << "EventBuilder_forward::AddBtag(): INFO The jetClass variable is empty. ";
    ...
    if (jetType.Contains("Truth")) {
      ...
      cout << "EventBuilder_forward::AddBtag(): ERROR Cannot add B-tagging for Truth jet collections!" << endl;
      return kFALSE;
      ...
      cout << "EventBuilder_forward::AddBtag(): ERROR Tower jets are not currently supported." << endl;
      return kFALSE;
    } // END IF
  ...
  // Otherwise, try to figure out what class of jet this really is
    ...
    cout << "EventBuilder_forward::AddBtag(): ERROR Currently, the only supported classes of jets are: truth, track, topo." << endl;
    return kFALSE;
  }
  ...
  if (fEvt->Debug()) {
    cout << "EventBuilder_forward::AddBtag(): DEBUG ";
    TString info = 
    ...
    if(doGbbCommon) jet->Set("flavor_weight_JetFitterCharm", Get<vector<float> >("jet_" + jetType + "_flavor_weight_JetFitterCharm").at(iJet));
    jet->Set("flavor_weight_MV1",                    Get<vector<float> >("jet_" + jetType + "_flavor_weight_MV1")                   .at(iJet));
    if(!doCOMMON && !doGbbCommon)  jet->Set("flavor_weight_MV2",                    Get<vector<float> >("jet_" + jetType + "_flavor_weight_MV2")                   .at(iJet));
    // TODO: Add MV2
    ...
    // (inclusive) svp
    if(doGbbCommon){
      jet->Set("svp_isValid",                          Get<vector<int> >  ("jet_" + jetType + "_flavor_component_svp_isValid")        .at(iJet));
      ...
      jet->Set("svp_mass",                             Get<vector<float> >("jet_" + jetType + "_flavor_component_svp_mass")           .at(iJet));
      jet->Set("svp_ntrk",                             Get<vector<int> >  ("jet_" + jetType + "_flavor_component_svp_ntrk")           .at(iJet));
      jet->Set("svp_chi2",                             Get<vector<float> >("jet_" + jetType + "_flavor_component_svp_chi2")           .at(iJet));
      jet->Set("svp_ndof",                             Get<vector<int> >  ("jet_" + jetType + "_flavor_component_svp_ndof")           .at(iJet));

      Point* svp = new Point();
      svp->x.SetXYZ(jet->Float("svp_x"), jet->Float("svp_y"), jet->Float("svp_z"));

      // add tracks associated to SV
      svp->AddVec("AssocTrk", true);
      for(int itrk = 0; itrk < Get<vector<int> >("jet_" + jetType + "_flavor_component_svp_trk_n").at(iJet); itrk++){
        int track_index = Get<vector<vector<int> > >("jet_" + jetType + "_flavor_component_svp_trk_index").at(iJet).at(itrk);

        if(track_index >= 0) svp->Add("AssocTrk", &fEvt->track(track_index));
      }

      jet->AddVec("svp");
      jet->Add("svp", svp);

      // add two-track vertex
      jet->AddVec("twotrackvtxs", true);
      for(int ivtx = 0; ivtx < Get<vector<int> >("jet_" + jetType + "_flavor_component_jftwotrackvertex_n").at(iJet); ivtx++){
        int vtx_index = Get<vector<vector<int> > >("jet_" + jetType + "_flavor_component_jftwotrackvertex_index").at(iJet).at(ivtx);
        if(vtx_index >= 0) jet->Add("twotrackvtxs", fEvt->Obj<Point>("twotrackvtxs", vtx_index));
      }

      // tracks associated to jet (for b-tagging)
      // assoctrk block (not sure what it really is)
      jet->AddVec("AssocTrk_Btag", true);
      for(int itrk = 0; itrk < Get<vector<int> >("jet_" + jetType + "_flavor_component_assoctrk_n").at(iJet); itrk++){
        int track_index = Get<vector<vector<int> > >("jet_" + jetType + "_flavor_component_assoctrk_index").at(iJet).at(itrk);
        //cout << track_index << endl;
        if(track_index >= 0)  jet->Add("AssocTrk_Btag", &fEvt->track(track_index));
      }

      //cout << "----- finish btag track -----" << endl;

      // ipplustrk block (tracks used for IP tagger)
      jet->AddVec("AssocTrk_IP", true);
      for(int itrk = 0; itrk < Get<vector<int> >("jet_" + jetType + "_flavor_component_ipplus_trk_n").at(iJet); itrk++){
        int track_index = Get<vector<vector<int> > >("jet_" + jetType + "_flavor_component_ipplus_trk_index").at(iJet).at(itrk);
        //cout << track_index << endl;
        if(track_index >= 0)  jet->Add("AssocTrk_IP", &fEvt->track(track_index));
      }

      //cout << "----- finish ip track -----" << endl;
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forward::AddBtag(): DEBUG Scheduled b-tags for jet collection = " << jetType << " with " << fEvt->jets(jetType) << " jets" << endl;
  return kTRUE;  
...
/// CopyJets
///=========================================
Bool_t EventBuilder_forward::CopyJets() 
{

  if (fEvt->Debug()) cout << "EventBuilder_forward: DEBUG In CopyJets() " << endl;   
  ...
  for(Int_t i = 0; i<fEvt->Cfg()->Objs(mJETTYPEVEC); ++i) {
    ...
    if(fEvt->Debug()) cout << "EventBuilder_forward::CopyJets(): DEBUG Copying jet type " << type << " which is the " << i << "th jet collection" << endl;
    if(fEvt->Debug()) cout << "EventBuilder_forward::CopyJets(): DEBUG type in vec is " << ((TObjString*) fEvt->Cfg()->Obj(mJETTYPEVEC,i))->GetString() << endl;
    if(type=="") continue;
    fail = fail && CopyJet(type,"");
    //delete type;
  }
  
  if (fEvt->Debug()) cout << "EventBuilder_forward::CopyJets(): DEBUG Scheduled All Jets" << endl;
  return fail;
}
...
///=========================================
Bool_t EventBuilder_forward::CopyJet(TString jetType, TString jetClass) 
{

  if (fEvt->Debug()) cout << "EventBuilder_forward: DEBUG In CopyJet() for jetType = " << jetType << "; jetClass = " << jetClass << endl;  
  
  // Handle the case where this is called without an argument
  if (jetType == "") { 
    Abort("EventBuilder_forward::CopyJet(): ERROR No jet type selected. Aborting!");
  }
  
  // Now try to figure out what type of jet this is
  if (jetClass == "") {
    if (fEvt->Debug()) {
      cout << "EventBuilder_forward::CopyJet(): INFO The jetClass variable is empty. ";
      cout << "Assuming that the class (truth, track, topo, etc) can be determined from the name." << endl;
    } // END IF
    ...
    if (jetType.Contains("Truth")) {
      if (fEvt->isMC()) jetClass = "truth";
        ...
        cout << "EventBuilder_forward::CopyJet(): ERROR This is not MC and you have requested truth jets. Reconfigure analysis and run again!." << endl;
        return kFALSE;
      }
    }
    else if (jetType.Contains("Track"))  jetClass = "track";
      ...
      cout << "EventBuilder_forward::CopyJet(): ERROR Tower jets are not currently supported." << endl;
      return kFALSE;
    } // END IF
  ...
  // Otherwise, try to figure out what class of jet this really is
    ...
    cout << "EventBuilder_forward::CopyJet(): ERROR Currently, the only supported classes of jets are: truth, track, topo." << endl;
    return kFALSE;
  }
  ...
  /// Spit out some useful information about this jet
  if (fEvt->Debug()) {
    cout << "EventBuilder_forward::CopyJet(): DEBUG ";
    TString info = 
    TString::Format("Jet %d: (E, Et, m, eta, phi) = (%1.2f, %1.2f, %1.2f, %1.2f, %1.2f)",
            iJet, 
            jet->p.E(),jet->p.Et(),jet->p.M(),jet->p.Eta(),jet->p.Phi()
  ...
  // Handle track jets
    ...
    jet->Set("constit_n", jet->Objs("constit"));
  }
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forward::CopyJet(): DEBUG Scheduled jet collection = " << jetType << " with " << fEvt->jets(jetType) << " jets" << endl;
  return kTRUE;  
  
} // END CopyJet

Bool_t EventBuilder_forward::BuildSubjetLink(){
  if(fEvt->Debug()) cout << "Entering EventBuilder_forward::BuildSubjetLink" << endl;

  TString prefix("jets");

  for(int itype = 0; itype < fEvt->Cfg()->Objs(mSUBJETTYPEVEC); itype++){
    TString subjetType = fEvt->Cfg()->Obj<TObjString>(mSUBJETTYPEVEC, itype)->GetString();

    for(int i = 0; i < fEvt->Cfg()->Objs(mJETTYPEVEC); i++){
      TString jetType = fEvt->Cfg()->Obj<TObjString>(mJETTYPEVEC, i)->GetString();

      if(jetType == subjetType) continue;
      if(!jetType.Contains(subjetType)) continue;

      // only jetType that is a groomed jet or sub-jet of "subjetType" can be retained
      TString GroomingType = TString(jetType(subjetType.Length(), jetType.Length()-subjetType.Length()).Data());

      // Now build the link
      for(int iJet = 0; iJet < fEvt->jets(subjetType); iJet++){ // parent jet loop
        Particle& Jet = fEvt->jet(iJet, subjetType);
        int nSubjets = Get<vector<int> >("jet_" + subjetType + "_" + GroomingType + "_n") .at(iJet);
        vector<int> mSubjets = Get<vector<vector<int> > >("jet_" + subjetType + "_" + GroomingType + "_index") .at(iJet);

        Jet.AddVec(prefix + GroomingType + "_Incl", true);
        for(int iSubjet = 0; iSubjet < nSubjets; iSubjet++){ // subjet loop
          Particle& Subjet = fEvt->jet(mSubjets[iSubjet], subjetType + GroomingType);

          Jet.Add(prefix + GroomingType + "_Incl", &Subjet);

          Subjet.AddVec("ParentJet", true);
          Subjet.Add("ParentJet", &Jet);
        }
      }

    } // end of loop on mJETTYPEVEC
  } // end of loop on mSUBJETTYPEVEC

  if(fEvt->Debug()) cout << "Leaving EventBuilder_forward::BuildSubjetLink" << endl;
...
/// CopyMuons
///=========================================
Bool_t EventBuilder_forward::CopyMuons()
{
...
/// CopyElectrons
///=========================================
Bool_t EventBuilder_forward::CopyElectrons() 
{
...
Bool_t EventBuilder_forward::DoHFOverlap(){
...
Bool_t EventBuilder_forward::MuonHasTriggerMatch(double mu_eta,
             double mu_phi,
...
Bool_t EventBuilder_forward::CopyMuonTriggers()
{
...
Bool_t EventBuilder_forward::CopyElectronTriggers()
{
...
Bool_t EventBuilder_forward::KillPileupFluctuations(){
  if(fEvt->isMC()){
    float maxpt6 = 0.;
...
Bool_t EventBuilder_forward::GetJetCalibrationTools(){
  cout << "In EventBuilder_forward::GetJetCalibrationTools(), doJetCalibrations: " << doJetCalibrations << endl;
    ...
    m_myJER = new JetSmearingTool(jetAlgo);//,jer_file);
...
Bool_t EventBuilder_forward::CopyStandardJet(int JetFlag){
  MomKey mjets;
  MomKey mjetsNoJ;
  BranchKey bjets;
  ...
  static const MomKey emscale_pt("emscale_pt"),
    ...
    TruthMFLinks("TruthMFLinks"), NumTowers("NumTowers");
...
Bool_t EventBuilder_forward::ParseJetStrings(){
  if(!fEvt->Cfg()->Exists(mJETTYPEVEC)) {
    ...
    TString jetType        = jetTypeObj->GetString();
        cout << "EventBuilder_forward::CopyEvent(): DEBUG Adding jet type: " << jetType << endl; 
      }
  ...
  if(!fEvt->Cfg()->Exists(mBTAGVEC)) {
    ...
    for(Int_t i = 0; i<arr->GetEntries(); ++i) {
      if (fEvt->Debug()) {
        TObjString* jetTypeObj = (TObjString*)arr->At(i);
    TString jetType        = jetTypeObj->GetString();
        cout << "EventBuilder_forward::CopyEvent(): DEBUG Adding btag type: " << jetType << endl; 
      }
  ...
  if(doGbbCommon){
    if(!fEvt->Cfg()->Exists(mSUBJETTYPEVEC)){
      fEvt->Cfg()->AddVec(mSUBJETTYPEVEC);
      TObjArray* arr = fEvt->Cfg()->String("SUBJETTYPES").Tokenize(",");
      for(Int_t i = 0; i < arr->GetEntries(); i++){
        if(fEvt->Debug()){
          TObjString* subjetTypeObj = (TObjString*)arr->At(i);
          TString subjetType = subjetTypeObj->GetString();
          cout << "EventBuilder_forward::CopyEvent() : DEBUG Adding Subjet type : " << subjetType << endl;
        }

        fEvt->Cfg()->Add(mSUBJETTYPEVEC, (TObjString*)arr->At(i));
      }
...
Bool_t EventBuilder_forward::AddPRWWeights(){
  if(doPRW){
...
Bool_t EventBuilder_forward::CopyJetTriggers(){
