...
#define EventBuilder_forwardnew_cxx
/// Type used for internal caching

#include "EventBuilder_forwardnew.h"
#include "AnaConfig.h"
...
EventBuilder_forwardnew::EventBuilder_forwardnew(TTree * /*tree*/)
{
...
EventBuilder_forwardnew::~EventBuilder_forwardnew(){
  // nothing to delete
  //delete m_trigdefs;
...
Bool_t EventBuilder_forwardnew::SetupPRW(){
  if(!fEvt->Cfg()->Exists(mPRWVEC)) {
    ...
    for(Int_t i = 0; i<arr->GetEntries(); ++i) {
      ...
      if (fEvt->Debug()) {
        cout << "EventBuilder_forwardnew::CopyEvent(): DEBUG Adding prw type: " << prwType << endl; 
      }
...
void EventBuilder_forwardnew::Initialize(){
  myGRL = (Root::TGoodRunsList*)fInput->FindObject("myGRL");  
  if(!myGRL) {
    cout << "EventBuilder_forwardnew: ERROR cannot retrieve GRL object from input list!" << endl;
    exit(1);
  }
...
Bool_t EventBuilder_forwardnew::CopyEvent(AnalysisBase* evt)
{
...
/// CopyMET
///=========================================
Bool_t EventBuilder_forwardnew::CopyMET(){
...
/// CopyTruthFull
///=========================================
Bool_t EventBuilder_forwardnew::CopyTruthFull()
{
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew: DEBUG CopyTruthFull()  and doTruth = " << doTruth << endl;  
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::CopyTruthFull(): DEBUG Scheduled Truth Particles" << endl;
  return kTRUE;
}
...
/// AddTruthParentChild
///=========================================
Bool_t EventBuilder_forwardnew::AddTruthParentChild()
{
  TString prefix;
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG Starting" << endl;

  if(fEvt->isMC() && doTruthParentChild)
  {
    for(int iTr = 0; iTr < fEvt->truths(); iTr++)
    {
      if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG On particle " << iTr << " of " << fEvt->truths() << endl;

      fEvt->truth(iTr).AddVec(children);
      fEvt->truth(iTr).AddVec(parents, true);
      if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG Has children " << Get<vector<vector<int> > >(bmc_children).at(iTr).size()  << endl;
      ...
      for(int iChild = 0; iChild < nChildren; iChild++)
      {
        ...
        if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG Adding child " << index  << endl;
      ...
      if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG Has parents " << Get<vector<vector<int> > >(bmc_parents).at(iTr).size()  << endl;
      ...
      for(int iParent = 0; iParent < nParents; iParent++)
      {
        ...
        if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG Adding parent " << index << endl;
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddTruthParentChild(): DEBUG Ending" << endl;
  return kTRUE;
...
Bool_t EventBuilder_forwardnew::CopyVertices()
{
...
Bool_t EventBuilder_forwardnew::CopyPhotons(){
  if(doPhotons){
    fEvt->AddVec("photons");
...
void EventBuilder_forwardnew::WriteAuxTrees(TDirectory* outfile){
...
/// AddBtags
///=========================================
Bool_t EventBuilder_forwardnew::AddBtags() 
{

  if (fEvt->Debug()) cout << "EventBuilder_forwardnew: DEBUG In AddBtags() " << endl;   
  bool fail = true;
  /// Copy all jet types
  for(Int_t i = 0; i<fEvt->Cfg()->Objs(mBTAGVEC); ++i) {
    ...
    if(fEvt->Debug()) cout << "EventBuilder_forwardnew::AddBtags(): DEBUG Copying btag type " << type << " which is the " << i << "th jet collection" << endl;
    if(fEvt->Debug()) cout << "EventBuilder_forwardnew::AddBtagss(): DEBUG type in vec is " << ((TObjString*) fEvt->Cfg()->Obj(mBTAGVEC,i))->GetString() << endl;
    fail = fail && AddBtag(type,"");
    //delete type;
  }
  
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddBtags(): DEBUG Scheduled All Jet B-tags" << endl;
  return fail;
}
...
/// AddBtag
///=========================================
Bool_t EventBuilder_forwardnew::AddBtag(TString jetType, TString jetClass) 
{

  if (fEvt->Debug()) cout << "EventBuilder_forwardnew: DEBUG In AddBtag() for jetType = " << jetType << "; jetClass = " << jetClass << endl;  
  
  // Handle the case where this is called without an argument
  if (jetType == "") { 
    Abort("EventBuilder_forwardnew::AddBtag(): ERROR No jet type selected. Aborting!");
  }
  
  // Now try to figure out what type of jet this is
  if (jetClass == "") {
    if (fEvt->Debug()) {
      cout << "EventBuilder_forwardnew::AddBtag(): INFO The jetClass variable is empty. ";
    ...
    if (jetType.Contains("Truth")) {
      ...
      cout << "EventBuilder_forwardnew::AddBtag(): ERROR Cannot add B-tagging for Truth jet collections!" << endl;
      return kFALSE;
      ...
      cout << "EventBuilder_forwardnew::AddBtag(): ERROR Tower jets are not currently supported." << endl;
      return kFALSE;
    } // END IF
  ...
  // Otherwise, try to figure out what class of jet this really is
    ...
    cout << "EventBuilder_forwardnew::AddBtag(): ERROR Currently, the only supported classes of jets are: truth, track, topo." << endl;
    return kFALSE;
  }
  ...
  if (fEvt->Debug()) {
    cout << "EventBuilder_forwardnew::AddBtag(): DEBUG ";
    TString info = 
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::AddBtag(): DEBUG Scheduled b-tags for jet collection = " << jetType << " with " << fEvt->jets(jetType) << " jets" << endl;
  return kTRUE;  
...
/// CopyJets
///=========================================
Bool_t EventBuilder_forwardnew::CopyJets() 
{

  if (fEvt->Debug()) cout << "EventBuilder_forwardnew: DEBUG In CopyJets() " << endl;   
  ...
  for(Int_t i = 0; i<fEvt->Cfg()->Objs(mJETTYPEVEC); ++i) {
    ...
    if(fEvt->Debug()) cout << "EventBuilder_forwardnew::CopyJets(): DEBUG Copying jet type " << type << " which is the " << i << "th jet collection" << endl;
    if(fEvt->Debug()) cout << "EventBuilder_forwardnew::CopyJets(): DEBUG type in vec is " << ((TObjString*) fEvt->Cfg()->Obj(mJETTYPEVEC,i))->GetString() << endl;
    if(type=="") continue;
    fail = fail && CopyJet(type,"");
    //delete type;
  }
  
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::CopyJets(): DEBUG Scheduled All Jets" << endl;
  return fail;
}
...
///=========================================
Bool_t EventBuilder_forwardnew::CopyJet(TString jetType, TString jetClass) 
{

  if (fEvt->Debug()) cout << "EventBuilder_forwardnew: DEBUG In CopyJet() for jetType = " << jetType << "; jetClass = " << jetClass << endl;  
  
  // Handle the case where this is called without an argument
  if (jetType == "") { 
    Abort("EventBuilder_forwardnew::CopyJet(): ERROR No jet type selected. Aborting!");
  }
  
  // Now try to figure out what type of jet this is
  if (jetClass == "") {
    if (fEvt->Debug()) {
      cout << "EventBuilder_forwardnew::CopyJet(): INFO The jetClass variable is empty. ";
      cout << "Assuming that the class (truth, track, topo, etc) can be determined from the name." << endl;
    } // END IF
    ...
    if (jetType.Contains("Truth")) {
      if (fEvt->isMC()) jetClass = "truth";
        ...
        cout << "EventBuilder_forwardnew::CopyJet(): ERROR This is not MC and you have requested truth jets. Reconfigure analysis and run again!." << endl;
        return kFALSE;
      }
    }
    else if (jetType.Contains("Track"))  jetClass = "track";
      ...
      cout << "EventBuilder_forwardnew::CopyJet(): ERROR Tower jets are not currently supported." << endl;
      return kFALSE;
    } // END IF
  ...
  // Otherwise, try to figure out what class of jet this really is
    ...
    cout << "EventBuilder_forwardnew::CopyJet(): ERROR Currently, the only supported classes of jets are: truth, track, topo." << endl;
    return kFALSE;
  }
  ...
  /// Spit out some useful information about this jet
  if (fEvt->Debug()) {
    cout << "EventBuilder_forwardnew::CopyJet(): DEBUG ";
    TString info = 
    TString::Format("Jet %d: (E, Et, m, eta, phi) = (%1.2f, %1.2f, %1.2f, %1.2f, %1.2f)",
            iJet, 
            jet->p.E(),jet->p.Et(),jet->p.M(),jet->p.Eta(),jet->p.Phi()
  ...
  if (fEvt->Debug()) cout << "EventBuilder_forwardnew::CopyJet(): DEBUG Scheduled jet collection = " << jetType << " with " << fEvt->jets(jetType) << " jets" << endl;
  return kTRUE;  
...
/// CopyMuons
///=========================================
Bool_t EventBuilder_forwardnew::CopyMuons()
{
...
/// CopyElectrons
///=========================================
Bool_t EventBuilder_forwardnew::CopyElectrons() 
{
...
Bool_t EventBuilder_forwardnew::DoHFOverlap(){
...
Bool_t EventBuilder_forwardnew::MuonHasTriggerMatch(double mu_eta,
             double mu_phi,
...
Bool_t EventBuilder_forwardnew::CopyMuonTriggers()
{
...
Bool_t EventBuilder_forwardnew::CopyElectronTriggers()
{
...
Bool_t EventBuilder_forwardnew::KillPileupFluctuations(){
  if(fEvt->isMC()){
    float maxpt6 = 0.;
...
Bool_t EventBuilder_forwardnew::GetJetCalibrationTools(){
  cout << "In EventBuilder_forwardnew::GetJetCalibrationTools(), doJetCalibrations: " << doJetCalibrations << endl;
...
Bool_t EventBuilder_forwardnew::CopyStandardJet(int JetFlag){
  MomKey mjets;
  MomKey mjetsNoJ;
  BranchKey bjets;
...
Bool_t EventBuilder_forwardnew::ParseJetStrings(){
  if(!fEvt->Cfg()->Exists(mJETTYPEVEC)) {
    ...
    TString jetType        = jetTypeObj->GetString();
        cout << "EventBuilder_forwardnew::CopyEvent(): DEBUG Adding jet type: " << jetType << endl; 
      }
  ...
  if(!fEvt->Cfg()->Exists(mBTAGVEC)) {
    ...
    for(Int_t i = 0; i<arr->GetEntries(); ++i) {
      if (fEvt->Debug()) {
        TObjString* jetTypeObj = (TObjString*)arr->At(i);
    TString jetType        = jetTypeObj->GetString();
        cout << "EventBuilder_forwardnew::CopyEvent(): DEBUG Adding btag type: " << jetType << endl; 
      }
...
Bool_t EventBuilder_forwardnew::AddPRWWeights(){
  if(doPRW){
...
Bool_t EventBuilder_forwardnew::CopyJetTriggers(){
